...
public class AdminUserTest {
    ...
    //注册收寄件人（初始状态 NORMAL），返回其 userRoleId
    ...
    void banKickoutAndUnban() throws Exception {
        Long roleId = registerCustomer("cus_ban", "13900000102");
        ...
        mockMvc.perform(post("/api/user/ban")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(String.format("{\"userRoleId\":%d,\"reason\":\"违规操作\"}", roleId))
                        .cookie(adminCookie))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(ResultCodeEnum.SUCCESS.getCode()));
        ...
        mockMvc.perform(post("/api/user/unban")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(String.format("{\"userRoleId\":%d}", roleId))
                        .cookie(adminCookie))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(ResultCodeEnum.SUCCESS.getCode()));
    ...
    void unbanNotBannedAccount() throws Exception {
        Long roleId = registerCustomer("cus_notban", "13900000103");
        mockMvc.perform(post("/api/user/unban")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(String.format("{\"userRoleId\":%d}", roleId))
                        .cookie(adminCookie))
    ...
    void banNonexistentUser() throws Exception {
        mockMvc.perform(post("/api/user/ban")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"userRoleId\":999999999999,\"reason\":\"x\"}")
                        .cookie(adminCookie))
                .andExpect(status().isOk())
    ...
    void adminKickoutUser() throws Exception {
        Long roleId = registerCustomer("cus_kick", "13900000301");
    ...
    void adminResetPassword() throws Exception {
        Long roleId = registerCustomer("cus_reset", "13900000501");
        Cookie customerCookie = login("13900000501", "1234567", "CUSTOMER");

        mockMvc.perform(post("/api/user/admin/reset-password")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(String.format("{\"userRoleId\":%d,\"newPassword\":\"adminset1\"}", roleId))
                        .cookie(adminCookie))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(ResultCodeEnum.SUCCESS.getCode()));
