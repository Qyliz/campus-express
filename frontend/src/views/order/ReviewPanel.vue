...
import { getReviews, createReview, createAppeal, appealStatusLabels, appealStatusTagType, reviewStatusLabels, reviewStatusTagType, type OrderReviews, type AppealStatusEnum } from '@/api/review'
...
<template>
  <section v-loading="loading" class="reviews">
    <el-space><h3>服务评价</h3><el-button link @click="load">刷新</el-button></el-space>
    <template v-if="data">
      <el-form v-if="data.canReview" label-position="top">
        <el-form-item label="服务评分"><el-rate v-model="rating" :disabled="submitting" /></el-form-item>
        <el-form-item label="评价内容">
          <el-input v-model="content" type="textarea" :rows="3" maxlength="500" show-word-limit :disabled="submitting" placeholder="请描述配送员的服务，提交后不可修改" />
        </el-form-item>
      ...
      <el-card v-for="item in data.reviews" :key="item.review.id" shadow="never" class="review">
        <el-space wrap>
          ...
          <el-tag :type="reviewStatusTagType[item.review.status]">{{ reviewStatusLabels[item.review.status] }}</el-tag>
        ...
        <div v-if="item.appeal" class="appeal">
          <el-tag :type="appealStatusTagType[item.appeal.status as AppealStatusEnum]">{{ appealStatusLabels[item.appeal.status as AppealStatusEnum] }}</el-tag>
    ...
    <el-dialog v-model="dialogOpen" title="申诉评价" width="min(500px, 90vw)" :close-on-click-modal="!submitting" :show-close="!submitting" :close-on-press-escape="!submitting">
      <p>每条评价只能申诉一次，请完整说明理由。</p>
      <el-input v-model="reason" type="textarea" :rows="4" maxlength="500" show-word-limit :disabled="submitting" />
      <template #footer>
...
.appeal { padding: 12px; background: var(--el-fill-color-light); margin-top: 12px; }
</style>
