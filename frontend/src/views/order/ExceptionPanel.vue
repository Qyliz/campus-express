...
import { exceptionTypes, reportException, resolveException, type DeliveryException } from '@/api/order'
...
const form = reactive({ type: 'CONTACT', resolution: 'RESUME' as 'RESUME' | 'CANCEL', description: '' })
...
<template>
  <section>
    ...
    <el-alert v-if="exceptions.some(e => e.status === 0)" title="异常待处理，配送已暂停。管理员处理后可继续配送或取消订单。" type="warning" :closable="false" class="notice" />
    <p v-if="!exceptions.length" class="muted">暂无异常记录</p>
    <el-timeline v-else class="history">
      <el-timeline-item v-for="item in exceptions" :key="item.id" :timestamp="formatDateTime(item.createTime)" placement="top" :type="item.status === 0 ? 'warning' : 'success'">
        <el-space wrap><b>{{ exceptionTypes[item.type] }}</b><el-tag :type="item.status === 0 ? 'warning' : 'success'">{{ item.status === 0 ? '待处理' : '已处理' }}</el-tag>
          <el-button v-if="isAdmin && item.status === 0" type="primary" size="small" @click="open(item.id)">处理异常</el-button>
        </el-space>
        <p class="description">上报说明：{{ item.description }}</p>
        <template v-if="item.status === 1">
          <p>处理结果：{{ item.resolution === 'CANCEL' ? '取消订单（已支付则模拟退款）' : '恢复配送' }}</p>
    ...
    <el-dialog v-model="visible" :title="target ? '处理异常' : '上报异常'" width="min(520px, 94vw)" :close-on-click-modal="!submitting" :close-on-press-escape="!submitting" :show-close="!submitting">
      <el-form label-position="top">
        <el-form-item v-if="!target" label="异常类型" required><el-select v-model="form.type"><el-option v-for="(label, value) in exceptionTypes" :key="value" :label="label" :value="value" /></el-select></el-form-item>
