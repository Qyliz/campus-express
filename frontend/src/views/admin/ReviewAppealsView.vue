    ...
    <el-form inline class="filter-form" @submit.prevent>
      <el-form-item label="申诉状态">
        <el-select v-model="filters.status" placeholder="全部" class="filter-control">
          ...
          <el-option
            v-for="o in appealStatusOptions"
            :key="o.value"
            :label="o.label"
            :value="o.value"
          />
    ...
    <el-table v-loading="loading" :data="rows" row-key="id" border stripe>
      ...
      <el-table-column label="订单编号" min-width="180">
        <template #default="{ row }"
          ><router-link :to="{ name: 'admin-order-detail', params: { id: row.orderId } }">{{
            row.orderId
      ...
      <el-table-column label="状态" width="110"
        ><template #default="{ row }"
          ><el-tag :type="appealStatusTagType[row.status as AppealStatusEnum]">{{
            appealStatusLabels[row.status as AppealStatusEnum]
          }}</el-tag></template
        ></el-table-column
      >
      ...
      <el-table-column label="操作" width="110"
        ><template #default="{ row }"
          ><el-button link type="primary" @click="open(row as ReviewAppeal)">{{
            row.status === 'PENDING' ? '查看并处理' : '查看结果'
          }}</el-button></template
        ></el-table-column
      >
    </el-table>
  ...
  >
    <div v-loading="detailLoading">
      <template v-if="original && selected">
        ...
        <el-tag :type="reviewStatusTagType[original.review.status]">{{
          reviewStatusLabels[original.review.status]
        }}</el-tag>
        <p class="text">原评价：{{ original.review.content }}</p>
        <p class="text">申诉理由：{{ selected.reason }}</p>
        <el-tag :type="appealStatusTagType[selected.status]">{{
          appealStatusLabels[selected.status]
        }}</el-tag>
    ...
    <template #footer>
      ...
      <el-button
        v-if="original && selected?.status === 'PENDING'"
        type="primary"
        :loading="submitting"
        @click="submit"
        >确认处理</el-button
